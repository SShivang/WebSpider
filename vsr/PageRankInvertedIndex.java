...
public class PageRankInvertedIndex extends InvertedIndex {
  ...
  private HashMap<String, Double> pageScores;

  /*
    This method is a new constructor in which the weight passed in by the
    user is used.
  */
  ...
  */
  ...
  protected void indexDocuments() {
    ...
    while (docIter.hasMoreDocuments()) {
      ...
      // if the document is equal to the page_rank.txt file
      // don't include it.
      if (!doc.file.getName().equals("page_ranks.txt")){
    ...
    // retrieve all the page rank scores from page_ranks.txt file
    pageScores = new HashMap<String, Double>();

    // try to parse in the page_rank file
    try (Scanner scan = new Scanner(new File(dirFile.toString() + "/page_ranks.txt"))){

    // Go through all the lines and add the value to the hashmap
    while (scan.hasNextLine()){
      Scanner lineScanner = new Scanner(scan.nextLine());
      String name = lineScanner.next();
      Double score = lineScanner.nextDouble();
      pageScores.put(name, score);
      }
  ...
  public Retrieval[] retrieve(HashMapVector vector) {
    ...
    for (Map.Entry<DocumentReference, DoubleValue> entry : retrievalHash.entrySet()) {
      ...
      // Go through the documents and retrieve the page rank score
      double pageRankScore = 0.0;
      if (pageScores.get(docRef.toString()) != null)
          pageRankScore = pageScores.get(docRef.toString());

      double score = entry.getValue().value;
      retrievals[retrievalCount++] = getRetrieval(queryLength, docRef, score) ;
      // add the page rank score to the normalized score
      retrievals[retrievalCount-1].score += pageRankScore * pageRankWeight;
    }
    ...
    for (int i = 0; i < args.length - 1; i++) {
      ...
      if (flag.equals("-html"))
        ...
        feedback = true;
          ...
          weight = Double.parseDouble(next);
          i++;
      }
      else {
